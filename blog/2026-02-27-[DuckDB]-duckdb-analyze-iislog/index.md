...
安裝完成後，您可以在終端機輸入 `duckdb` 來啟動交互式命令列介面。

---

## 4. 實戰：匯入 IIS Log 到 DuckDB
...
3. **VS Code SQLTools**：在 VS Code 內直接執行 SQL 查詢，無需切換工具。
    * 須搭配 DuckDB 的 SQLTools Driver。
4. **Rill Developer**：能一秒將 DuckDB 轉為互動式 Dashboard 的神器。
    * Gemini 推薦的，我沒用過。
